%a demo for the fixed label threshlding version of FALT.
...
%%hyperparameters for linear FFLT
eta = 2.^(-6);
maxIter = 80;
...
for run = 1:times
    index = randperm(n);
    w = FFLT_train_sparse(trainData',trainLabel', index, epoch, eta, maxIter, gamma);
    tic
...
fprintf(fid,'name = bibtex, FFLT, runTimes = %d, epoch = %d, eta = %g, maxIter = %d, gamma = %g \n', times, epoch, eta, maxIter, gamma);
...
fid1 = fopen('appendix/FFLT_bibtex_appendix.txt','a');
for run = 1:times
