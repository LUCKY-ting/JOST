        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *data, *labels, *w, *index, *x, *y, eta,t, a_t, b_t, coeff, gamma;
            ...
            for (iter = 0; iter < maxIterNum; iter++){
                ...
                for (k = 0; k < L; k++){  // update the predictive models
                    if (y[k] == 1 && pred_v[k] - gamma < 1){
                        ...
                        pred_v[k] = 0;
                        for (p = 0; p < nonzerosNum; p++){
                            w[k*d + idx[p]] = w[k*d + idx[p]] + eta/Y_t_size * x[p]; // update the model
                            pred_v[k] += w[k*d + idx[p]] * x[p]; // update its prediction
                        }
                    }else if (y[k]==-1 && gamma - pred_v[k] < 1){
                        b_t++;
                        pred_v[k] = 0;
                        for (p = 0; p < nonzerosNum; p++){
                            w[k*d + idx[p]] = w[k*d + idx[p]] - eta/n_Y_t_size * x[p];// update the model
                            pred_v[k] += w[k*d + idx[p]] * x[p]; // update its prediction
                        }
                    }
                }
