        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *data, *labels, *w, *index, *x, *y, eta,t, a_t, b_t, coeff, *s_t, *H_t,delta;
    ...
    s_t = Malloc(double,d * (L+1));
    H_t = Malloc(double,d * (L+1));
    for (k = 0; k < d * (L+1); k++){
        H_t[k] = delta;
        s_t[k]= 0;
    }
    ...
    for (o = 1; o <= epoch; o++){
        ...
        for(i = 0; i < N; i++)
        {
            ...
            for (iter = 0; iter < maxIterNum; iter++){
                ...
                pred_v[L] = 0;
                for (p = 0; p < nonzerosNum; p++){
                    s_t[L*d + idx[p]] = sqrt(pow(s_t[L*d + idx[p]], 2) + pow(coeff * x[p], 2));
                    H_t[L*d + idx[p]] = delta + s_t[L*d + idx[p]];
                    w[L*d + idx[p]] = w[L*d + idx[p]] - (eta / H_t[L*d + idx[p]])* coeff * x[p];
