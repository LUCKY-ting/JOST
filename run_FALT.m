%used for datasets with sparse features and labels

clc
clear
global testData testLabel L;
load('datasets/rcv1_topics-train.mat');
load('datasets/rcv1_topics-test.mat');
[n,d] = size(trainData);
L = size(trainLabel,2);
...
%%hyperparameters for linear FALT
eta = 2.^(0);
maxIter = round(51);
...
for run = 1:times
    index = randperm(n);
    w = FALT_train_sparse(trainData',trainLabel', index, epoch, eta, maxIter);
    tic
...
fid = fopen('FALT_result.txt','a');
fprintf(fid,'name = rcv1_topics, FALT, runTimes = %d, epoch = %d, eta = %g, maxIter = %d \n', times, epoch, eta, maxIter);
...
fid1 = fopen('appendix/FALT_rcv1_topics_appendix.txt','a');
for run = 1:times
