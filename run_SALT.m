...
for run = 1:times
    index = randperm(n);
    w = SALT_train_sparse(trainData',trainLabel', index, epoch, eta,delta, maxIter);
    tic
...
fid = fopen('SALT_result.txt','a');
fprintf(fid,'name = bibtex, SALT, runTimes = %d, epoch = %d, eta = %g, delta = %g, maxIter = %d \n', times, epoch, eta, delta, maxIter);
...
fid1 = fopen('appendix/SALT_bibtex_appendix.txt','a');
for run = 1:times
